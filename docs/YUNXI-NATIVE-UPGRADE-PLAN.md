...
也不冒充真实发行版文档的最终质量。下一阶段继续扩展真实 `man/help` 文本、风险标签、
版本差异和延迟指标。
...
是否存在、active generation、FTS/向量证据数量、`source_version`、本轮检索耗时和最多 8 条
不含正文的 `knowledge_provenance`；记忆继续通过独立的 `MemoryRecall` 事件和上下文元数据记录
