# Miyu × YunXi Linux 大融合计划

## 目标

把 Miyu 已经验证过的 Linux 原生宿主能力迁入 YunXi，同时让 YunXi 继续作为唯一的对话、人格、记忆、陪伴、工具审批和数据边界真相源。
...
因此，Miyu 的 Linux 宿主实现可以优先采用，但不能绕过 YunXi 的审批、记忆敏感度、工作区和人格边界。

## 迁移层次

### 第一层：直接迁移 Miyu 的 Linux 宿主能力
...
不直接迁移 Miyu 的角色人格、记忆表结构和 Web/QQ/语音平台实现。
...
Miyu 使用 MIT License。直接迁移或实质改写 Miyu 源码时：
...
在继续融合前，已对固定版本的 Miyu checkout 做全文件清单和关键路径源码审计，见 [`MIYU-SOURCE-AUDIT.md`](MIYU-SOURCE-AUDIT.md) 与 [`audit/miyu-file-inventory.csv`](audit/miyu-file-inventory.csv)。审计确认 daemon、Web 宿主、运行时、IPC、fish hook 和会话生命周期相互耦合；因此“复制 daemon.rs”或“基础 hook 能跑”都不能视为迁移完成。
...
下一步只能按审计报告的验收门推进：先协议与 daemon 生命周期，再真实 fish PTY，最后逐工具选择性迁移。任何新模块都必须标注“保留 YunXi / adapter / 包裹 / 重写 / 暂缓”，不得以数量驱动合并。
