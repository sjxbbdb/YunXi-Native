...
的 Rust 实现已删,功能由 `src/personas/default/scripts/` 下同名 Python 脚本提供(09-23 前在 `src/scripts/personas/default/`)。
...
- 隔离 `MIYU_HOME` 与 **`XDG_RUNTIME_DIR`**(工具桥先找 daemon 的 IPC socket;不隔离运行目录
  ...
  `MIYU_SYSTEM_SCRIPTS_DIR` 指向仓库 `src/scripts`,不吃已安装的 `/usr/share/miyu/…`。这个目录 09-23 起已不存在,
  变量只当资源树的锚用:出厂脚本与技能从它的上一级 `src/personas/` 读。
