...
pub(in crate::web) fn prepare_web_attachment_data(
    display_content: &str,
    attachments: Vec<miyu_core::state::UserAttachmentData>,
) -> std::result::Result<PreparedWebAttachments, ApiError> {
    ...
    for attachment in attachments {
        ...
        if attachment.attachment.kind == USER_ATTACHMENT_KIND_FILE {
            ...
            if miyu_engine::tools::vision::pdf_mime(&attachment.attachment.file_name).is_some() {
...
impl AttachmentRunGuard {
    pub fn new(store: StateStore, run_id: Option<String>) -> Self {
        Self { store, run_id }
    }
}
...
pub(in crate::web) use test_support::*;
