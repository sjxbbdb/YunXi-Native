...
use crate::web::*;
use miyu_base::config::feature_catalog;

fn account_json(account: &miyu_core::state::Account) -> Value {
    json!({
        "id": account.id,
    ...
    if !identity.admin && !identity.username.is_empty() {
        ...
        if setup_pending {
            value["setup_username"] = json!(state
                .paths
                .home_admin()
                .unwrap_or_else(|| miyu_core::state::BOOTSTRAP_ADMIN_USERNAME.to_string()));
        }
    }
    value
}
...
pub(in crate::web) async fn account_update(
    ...
    if let Some(profile) = request.profile.as_deref() {
        ...
        if let Some(parent) = path.parent() {
            miyu_base::paths::ensure_private_dir(parent).map_err(ApiError::internal)?;
        }
    ...
    if let Some(password) = request.password.as_deref() {
        ...
        if !miyu_core::state::verify_password(&account.password_hash, current) {
            return Err(ApiError::new(
...
fn invite_json(invite: &miyu_core::state::Invite, now: &str) -> Value {
...
pub(in crate::web) async fn admin_create_invite(
    ...
    let created_by = if identity.account_id.is_empty() {
        miyu_core::state::BOOTSTRAP_ADMIN_USERNAME.to_string()
    } else {
        identity.account_id.clone()
    };
    let (code, invite) = state
        .state_store
        .create_invite(&created_by, request.days, miyu_core::state::ROLE_MEMBER)
        .map_err(ApiError::internal)?;
    let now = chrono::Utc::now().to_rfc3339();
    Ok((
        StatusCode::CREATED,
...
pub(in crate::web) async fn account_personas(
    ...
    let options = member_persona::member_selectable_plugins(&config)
        ...
        .collect::<Vec<_>>();
    let scripts = miyu_engine::tools::list_global_scripts(&state.paths)
        .into_iter()
        .map(
    ...
    let skills = miyu_core::skills::persona_skill_options(
...
fn retarget_member_sessions(state: &DaemonState, identity: &WebIdentity, scope: &str) {
    ...
    for overview in sessions.iter().filter(|overview| overview.turn_count == 0) {
        ...
        if overview.record.persona == miyu_core::state::DEV_PERSONA {
            continue;
        }
    ...
    if let Ok(record) =
        store.create_session_for_owner(scope, "", miyu_core::state::USER_SESSION_KIND, None, owner)
    {
