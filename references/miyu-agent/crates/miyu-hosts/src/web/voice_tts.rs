...
pub(crate) fn strip_speak(reply: &str) -> String {
    miyu_engine::agent::prompt_strip_tagged(reply.to_string(), "speak")
        .trim()
        .to_string()
}
...
pub(crate) fn spoken_text(reply: &str, tts: &VoiceTtsConfig) -> String {
    ...
    let text = text.trim();
    if text.is_empty() {
        return miyu_base::i18n::text("done", "办好了").to_string();
    }
...
pub async fn list_minimax_voices(tts: &MiniMaxTtsConfig) -> Result<Vec<Value>> {
    let (client, auth) = minimax_client(tts)?;
    let response = client
