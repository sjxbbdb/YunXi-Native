...
fn memory_store(
    ...
    persona: &str,
) -> std::result::Result<miyu_core::memory::MemoryStore, ApiError> {
    let config = super::dash_memory_config_for(state, identity, persona)?;
    Ok(miyu_core::memory::MemoryStore::new(&config, &state.paths))
}
...
pub(in crate::web) async fn dash_memory_personas(
    ...
    let active = miyu_base::config::persona_scope_name(&config.prompt.active_persona);
...
pub(in crate::web) async fn dash_memory_reset(
    ...
    blocking(move || store.reset_all()).await?;
    Ok(Json(json!({ "ok": true })))
}
