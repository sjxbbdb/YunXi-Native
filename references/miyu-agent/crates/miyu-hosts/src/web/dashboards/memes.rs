...
fn library_for(
    ...
    let own = miyu_engine::tools::memes::current_persona_library(&config);
    let requested = requested.trim();
    if !requested.is_empty() && miyu_engine::tools::memes::sanitize_library(requested) != own {
        return Err(ApiError::new(
            StatusCode::FORBIDDEN,
...
fn library_name(state: &DaemonState, requested: &str) -> String {
    ...
    if requested.is_empty() {
        ...
        miyu_engine::tools::memes::current_persona_library(&config)
    } else {
        miyu_engine::tools::memes::sanitize_library(requested)
    }
}
