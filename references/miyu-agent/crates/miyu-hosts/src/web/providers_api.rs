...
pub(in crate::web) struct ProviderModelsResponse {
    ...
    pub(in crate::web) models: Vec<miyu_base::models_cache::ModelCatalogEntry>,
}
...
pub(in crate::web) async fn provider_models(
    ...
    let result = tokio::task::spawn_blocking(move || -> anyhow::Result<ProviderModelsResponse> {
        let (source, ids) = if fetch {
            let cli_binary = miyu_base::provider_catalog::builtin_cli_binary(&current, &provider);
            let ids = miyu_base::provider_catalog::fetch_models(&provider, cli_binary.as_deref())?;
            (
        ...
        let ids: Vec<String> = ids
            ...
            .collect();
        let models = miyu_base::models_cache::describe_models(
            &paths,
            &provider.id,
            &provider.base_url,
            &ids,
