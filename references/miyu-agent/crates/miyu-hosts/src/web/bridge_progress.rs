...
pub(in crate::web) async fn call_with_bridge_progress(
    state: &DaemonState,
    session_id: &str,
    registry: &miyu_engine::tools::ToolRegistry,
    name: &str,
    arguments: &str,
) -> Result<String> {
    ...
    let future = registry.call_with_progress_future(
        ...
        arguments,
        sender,
        &miyu_engine::tools::GuardCtx::default(),
    )?;
    tokio::pin!(future);
...
fn handle_bridge_progress(
    ...
    tool_name: &str,
    image_seq: &mut usize,
    event: miyu_engine::tools::ToolProgressEvent,
) {
    match event {
        miyu_engine::tools::ToolProgressEvent::Image { path, alt, size } => {
            *image_seq += 1;
        ...
        // 没有)。
        miyu_engine::tools::ToolProgressEvent::Artifact { path, title } => {
            *image_seq += 1;
        ...
        miyu_engine::tools::ToolProgressEvent::PrepareForExternalOutput { ready } => {
