...
impl PlatformFollowupRun {
    pub fn new(context: Arc<PlatformTurnContext>) -> Arc<Self> {
        Arc::new(Self {
    ...
    pub fn try_reserve(&self) -> Option<QueueIngressReservation> {
...
pub struct PlatformTurnContext {
...
impl miyu_engine::agent::PlatformTurn for PlatformTurnContext {
    fn platform_name(&self) -> &str {
        self.conversation.platform.as_str()
    }

    fn take_queued_files(&self, prompt_id: &str) -> Vec<PlatformContextFileRef> {
        PlatformTurnContext::take_queued_files(self, prompt_id)
    }
...
impl PlatformTurnContext {
        ...
        let delivered = match self.adapter.send(primary.clone()).await {
            ...
            Err(error) => {
                ...
                match (partially_delivered, prepared.fallback) {
                    ...
                    (false, Some(fallback)) => {
                        tracing::warn!(target: "miyu::qq", error = %error, "{}", miyu_base::i18n::text("transformed platform message failed; sending fallback", "转换后的平台消息发送失败；正在发送回退消息"));
        ...
        for message in prepared.after_success {
            ...
            match self.adapter.send(message).await {
                ...
                Err(error) => {
                    ...
                    tracing::warn!(target: "miyu::qq", error = %error, "{}", miyu_base::i18n::text("platform plugin follow-up send failed", "平台插件后续消息发送失败"));
                }
            }
        }
    ...
    pub(crate) async fn bot_send_availability(
        &self,
    ) -> miyu_base::platform_types::BotSendAvailability {
        match self.adapter.bot_send_availability().await {
            Ok(availability) => availability,
            Err(error) => {
                tracing::debug!(error = %error, "{}", miyu_base::i18n::text("platform bot send availability lookup failed", "平台机器人发送可用性查询失败"));
                miyu_base::platform_types::BotSendAvailability::Unknown
            }
        }
    }
    ...
    pub(crate) async fn bot_group_role(&self) -> miyu_base::platform_types::BotGroupRole {
        self.adapter
            .bot_group_role()
            .await
            .unwrap_or(miyu_base::platform_types::BotGroupRole::Unknown)
    }
