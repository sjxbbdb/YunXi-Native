...
pub(in crate::platforms::onebot) async fn attach_voice_transcripts(
    ...
    let stt_ready = crate::runtime::voice_port().is_some_and(|port| port.stt_available());
    ...
    for media in voices.iter().take(MAX_VOICE_SEGMENTS) {
        ...
        lines.push(if transcript.trim().is_empty() {
            ...
            miyu_base::platform_types::voice_history_text(&transcript)
        });
    }
    ...
    for source in sources {
        ...
        match bytes {
            Ok(bytes) if !bytes.is_empty() => {
                return crate::runtime::voice_port()
                    .context("voice transcription needs the daemon voice bridge")?
                    .transcribe_wav(bytes)
                    .await;
            }
