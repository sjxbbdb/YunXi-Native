...
impl SessionTurnState {
    pub fn new(limits: PlatformSessionLimits) -> Self {
        Self {
...
pub(crate) struct TurnProfile {
    ...
    pub(crate) group_context: Option<miyu_base::config::PlatformGroupContextConfig>,
...
pub(crate) fn resolve_platform_session(
    ...
    if candidates.len() == 1 {
            ...
            .claim_platform_session(&key, &record.session_id)
        {
            ...
            Err(error) => {
                tracing::warn!(error = %error, session_id = %record.session_id, "{}", miyu_base::i18n::text("legacy platform session could not be bound", "无法绑定旧版平台会话"));
                if let Some(session_id) = state.state_store.find_platform_session_binding(&key)? {
