...
use miyu_core::state::PlatformSessionBinding;
...
pub use real_context::active_judgement_skip::{
    active_judgement_skip_ids, apply_active_judgement_skip_editor_changes,
};
pub use renderer::{renderer_worker_requested, run_renderer_worker};
...
pub struct PlatformPersonaResetContext<'a> {
    pub config: &'a AppConfig,
    pub paths: &'a MiyuPaths,
    pub bindings: &'a [PlatformSessionBinding],
}
...
#[derive(Default)]
pub struct PlatformPluginRegistry {
...
impl PlatformPluginRegistry {
    ...
    pub fn new(mut plugins: Vec<Arc<dyn PlatformPlugin>>) -> Self {
        ...
        for plugin in self.enabled_plugins(context) {
            match plugin.handle_command(context, text).await {
                ...
                Err(error) => tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin command failed", "平台插件命令处理失败")
                ),
            }
        }
        None
        ...
        for plugin in self.enabled_plugins(&context) {
            if let Err(error) = plugin.register_tools(registry, context.clone()) {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin tool registration failed", "平台插件工具注册失败")
                );
            }
        }
    }
        ...
        for plugin in self.enabled_plugins(context) {
            match plugin.preempt_inbound(context, event) {
                ...
                Err(error) => tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin inbound preemption failed", "平台插件入站抢占失败")
                ),
            }
        }
        cancel_active_turn
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.after_turn_aborted(context).await {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin aborted-turn cleanup failed", "平台插件中止轮次清理失败")
                );
            }
        }
    }
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.observe_inbound(context, event).await {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin inbound observer failed", "平台插件入站观察器失败")
                );
            }
        }
    }
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.accept_followup(context, event) {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin follow-up hook failed", "平台插件后续消息钩子失败")
                );
            }
        }
    }
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.decide_trigger(context, event, decision).await {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin trigger hook failed", "平台插件触发钩子失败")
                );
            }
        }
    }
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.after_session_reset(context).await {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin post-reset hook failed", "平台插件重置后钩子失败")
                );
    ...
    pub async fn after_persona_reset(
        &self,
            ...
            match plugin.before_send(context, prepared.primary).await {
                ...
                Err(error) => {
                    tracing::warn!(
                        ...
                        error = %error,
                        "{}",
                        miyu_base::i18n::text("platform plugin before-send hook failed", "平台插件发送前钩子失败")
                    );
        ...
        for plugin in self.enabled_plugins(context) {
            if let Err(error) = plugin.after_send(context, message, receipt).await {
                tracing::warn!(
                    ...
                    error = %error,
                    "{}",
                    miyu_base::i18n::text("platform plugin after-send hook failed", "平台插件发送后钩子失败")
                );
            }
        }
    }
