...
use super::store::{
    ActivityRankingQuery, ConversationKey, DeleteMode, DeleteRequest, HistoryMessage, HistoryScope,
    HistoryStore, MediaKind, RecentQuery, SearchQuery,
};
...
use miyu_base::config::QqMessageHistoryPluginSettings;
...
fn register_activity_ranking(
    ...
    registry.register(
        ...
        .with_display_name(miyu_base::i18n::text("Rank group activity", "群活跃榜")),
    );
}
...
pub(super) fn register_group_members(
    ...
    registry.register(
        ...
        .with_display_name(miyu_base::i18n::text("Query group members", "查询群成员")),
    );
}
...
pub(super) fn register_avatar(registry: &mut ToolRegistry, context: Arc<PlatformTurnContext>) {
    registry.register(
        ...
        .with_display_name(miyu_base::i18n::text("QQ avatar", "QQ 头像")),
    );
}
