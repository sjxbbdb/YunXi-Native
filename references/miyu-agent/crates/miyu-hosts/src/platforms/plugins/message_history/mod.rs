...
use miyu_base::config::{QqMessageHistoryPluginSettings, QQ_MESSAGE_HISTORY_PLUGIN_ID};
...
impl PlatformPlugin for MessageHistoryPlugin {
    ...
    fn observe_ingress<'a>(
        &'a self,
        paths: &'a MiyuPaths,
        _config: &'a miyu_base::config::AppConfig,
    ...
    for segment in segments {
        match segment {
            ...
            OutboundSegment::AudioPath { transcript, .. } => {
                parts.push(miyu_base::platform_types::voice_history_text(transcript))
            }
        }
    }
}
