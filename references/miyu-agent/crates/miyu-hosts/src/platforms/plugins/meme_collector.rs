...
use miyu_base::config::{QqMemeCollectorPluginSettings, QQ_MEME_COLLECTOR_PLUGIN_ID};
use miyu_core::state::PlatformMemeRefRecord;
            ...
            while let Some(job) = receiver.recv().await {
                if let Err(error) = process_collection_job(job).await {
                    tracing::warn!(
                        %error,
                        "{}",
                        miyu_base::i18n::text("QQ meme collection job failed", "QQ 表情包收集任务失败")
                    );
                }
            }
        });
