...
use miyu_base::config::REAL_CONTEXT_PLUGIN_ID;
use miyu_core::state::{PlatformPluginScopeKey, StateStore, GLOBAL_PLATFORM_ACCOUNT_SCOPE};
...
pub fn active_judgement_skip_ids(state: &StateStore) -> Result<Vec<i64>> {
...
pub fn apply_active_judgement_skip_editor_changes(
    state: &StateStore,
    original: &[i64],
    edited: &[i64],
) -> Result<Vec<i64>> {
...
pub(super) fn register_tools(registry: &mut ToolRegistry, context: Arc<PlatformTurnContext>) {
    ...
    registry.register(
        ...
        .writes()
        .with_display_name(miyu_base::i18n::text("Skip proactive judgement", "添加跳过主动判断 QQ 号")),
    );
    registry.register(
        ToolSpec::new(
        ...
        .writes()
        .with_display_name(miyu_base::i18n::text("Unskip proactive judgement", "移除跳过主动判断 QQ 号")),
    );
}
...
mod tests {
    ...
    use miyu_core::state::PlatformAccessActor;
    use miyu_engine::tools::ToolPermission;
    use std::path::PathBuf;
    use std::sync::Mutex;
