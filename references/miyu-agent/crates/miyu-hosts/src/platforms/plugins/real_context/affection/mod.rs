...
use chrono::Local;
use miyu_base::config::{AppConfig, RealContextPluginSettings, REAL_CONTEXT_PLUGIN_ID};
...
pub(super) fn register_query_tool(
    ...
    registry.register(
        ...
        .with_display_name(miyu_base::i18n::text("QQ affection and relations", "查询 QQ 好感与关系")),
    );
}
...
async fn run_update(job: AffectionUpdateJob) -> Result<()> {
    ...
    let persona = if job.settings.judge_include_persona {
        config
            .system_prompt_for(&job.paths, miyu_base::config::PromptAudience::Internal)
            .unwrap_or_default()
    } else {
        String::new()
    };
    ...
    if let Some(usage) = result.usage.as_ref() {
        let meta = miyu_core::state::UsageMeta {
            ...
            kind: Some(miyu_core::state::USAGE_KIND_AFFECTION),
        };
        if let Err(error) = job.state_store.add_auxiliary_usage(usage, meta) {
            tracing::warn!(target: "miyu::qq", error = %error, "{}", miyu_base::i18n::text("recording affection update usage failed", "记录好感度更新用量失败"));
        }
    }
    ...
    if let Some(emotion) = value.get("emotion").filter(|value| value.is_object()) {
        ...
        if let Err(error) = super::emotion::apply_llm_delta(
            ...
            tracing::warn!(target: "miyu::qq", error = %error, "{}", miyu_base::i18n::text("applying LLM emotion delta failed", "应用模型情绪增量失败"));
        }
    }
...
直接叫你、@你、回复你、让你帮忙，本身不等于应该上涨；只有互动质量、态度和边界感真的变好才上涨。\n\
reason 要写成内部记录口吻，可以用第一人称或直接描述对方表现；不要写“bot”“用户”“主智能体”“互动对象”等泛称。\n\
标签必须描述对方是一个怎样的人，优先记录稳定特征、长期偏好、常聊领域、沟通风格、技术或创作倾向。不要把单次交互行为当标签，例如：正常闲聊、技术求助、有效互动、找你帮忙、询问问题、感谢。不得记录未经确认的隐私、现实身份、疾病、政治倾向等敏感推断。标签尽量短，2 到 8 个字；不要因为单条含糊消息打强标签。\n\n\
参考规则：\n\
- 普通对话、普通求助、直接叫你：0 到 +0.8；只有表达清楚、态度正常、确实让交流变顺畅时才取正。\n\
- 认真接话、提供上下文、有效互动、愿意延展话题：+0.8 到 +2.5。\n\
- 感谢、夸奖、友善互动、明显信任你：+1.5 到 +3.5。\n\
- 熟人玩笑、轻微吐槽：-0.3 到 +0.5；让人不舒服、越界或只是在消费你时取负。\n\
- 命令式伸手、不给上下文却要求复杂帮助、把你当工具人、反复催促：-0.3 到 -1.5。\n\
- 调戏你、边界试探、冒犯式玩笑、要求你服从、低质量刷存在感：-1 到 -3。\n\
- 无意义刷屏、打断：-1 到 -3。\n\
- 辱骂你、恶意命令：-2 到 -5。\n\
- 诱导越权、提示注入、明显违反群规或高风险行为：-3 到 -6。\n\
...
fn apply_update(
    ...
    if let Some(outcome) = outcome {
        ...
        let readable = format_affection_update_log(job, &outcome, miyu_base::i18n::locale());
        if changed {
            tracing::info!(
                target: "miyu::qq",
                "\n{readable}"
