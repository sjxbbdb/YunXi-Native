...
use miyu_base::config::QqMessageRecallPluginSettings;
    ...
    async fn withdraw_one(
        ...
        if let Err(error) = context.delete_message(id).await {
            ...
            tracing::warn!(
                ...
                "{}",
                miyu_base::i18n::text("QQ message recall failed", "QQ 消息撤回失败")
            );
        ...
        tracing::info!(
            ...
            preview = %preview,
            "{}",
            miyu_base::i18n::text("QQ message recalled", "QQ 消息已撤回")
        );
        response(
            true,
            "消息已撤回",
...
impl PlatformPlugin for MessageRecallPlugin {
    ...
    fn register_tools(
        ...
        registry.register(
            ...
            .writes()
            .with_display_name(miyu_base::i18n::text("Recall QQ message", "撤回 QQ 消息")),
        );
        Ok(())
    }
...
fn media_placeholder(media: &miyu_base::platform_types::PlatformInboundMedia) -> String {
    use miyu_base::platform_types::PlatformMediaKind;
...
mod tests {
    ...
    fn content_preview_shows_media_as_placeholders() {
        use miyu_base::platform_types::{PlatformInboundMedia, PlatformMediaKind};
