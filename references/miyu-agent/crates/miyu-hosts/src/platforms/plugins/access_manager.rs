...
impl PlatformPlugin for AccessManagerPlugin {
    ...
    fn register_tools(
        ...
        registry.register(
            ...
            .writes()
            .with_display_name(miyu_base::i18n::text("Manage platform access", "管理通讯平台权限")),
        );
        Ok(())
    }
}
...
fn format_permission_list(
    config: &miyu_base::config::OneBotConfig,
    account_id: &str,
...
mod tests {
    ...
    fn registered_tool_is_visible_to_the_model_and_is_a_write_operation() {
        ...
        assert_eq!(tool.permission, miyu_engine::tools::ToolPermission::Writes);
