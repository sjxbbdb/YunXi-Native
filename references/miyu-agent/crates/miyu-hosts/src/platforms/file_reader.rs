...
pub fn register(
    ...
    let description = if state.context.conversation.kind
        == miyu_base::platform_types::ConversationKind::Group
    {
    ...
    registry.register(
        ...
        .with_display_name(miyu_base::i18n::text("Read uploaded file", "读取上传文件")),
    );
}
...
fn inline_platform_pdf(state: &FileReaderState, path: &Path, name: &str, size: u64) -> String {
    ...
    if miyu_engine::agent::active_text_pool_supports_pdf(&state.context.config) {
        match miyu_engine::tools::vision::pdf_inline_media(&display.to_string()) {
            Ok(items) => return miyu_engine::tools::vision::inline::deposit(items),
            Err(error) => {
                return format!(
...
fn visual_file_hint(name: &str, reference: &str) -> Option<String> {
    ...
    let kind = if miyu_engine::tools::vision::video_mime(name).is_some() {
        "a video"
