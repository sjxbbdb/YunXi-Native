...
impl Agent {
    pub(in crate::agent) async fn prepare_user_input(
        ...
        let path_images = images
            ...
            .collect::<Vec<_>>();
        let absolute_image_paths = resolve_pasted_image_paths(
            images,
            &self.core.paths,
            self.input.image_platform.as_deref(),
        );
        ...
        if self.core.tools_enabled
            && (self.core.config.plugins.vision.enabled
                || self.core.config.plugins.image_generation.enabled)
            && self.input.image_platform.is_some()
        {
            let mut tools = self.tools.lock().unwrap();
            if let Some(platform_context) = self.input.platform_context.clone() {
                vision::register_scoped_platform(
                    &mut tools,
                    self.core.config.clone(),
                    self.core.paths.clone(),
                    binary_paths.iter().map(PathBuf::from).collect(),
                    self.input.context_images.clone(),
                    self.input.context_files.clone(),
                    platform_context,
                );
        ...
        let vision_tool_available =
            self.core.tools_enabled && self.tools.lock().unwrap().contains("vision_analyze");
        ...
        let native_media_viewer = self
            .core
            .config
            .active_pool_views_media_with_native_file_tool(self.core.dev);
        ...
        if !binary_paths.is_empty() {
            let source = self
                ...
                .as_deref()
                .or(self.input.image_platform.as_deref())
        ...
        if !self.input.context_images.is_empty() && !vision_tool_available {
            tracing::warn!(
                target: "miyu::qq",
                refs = self.input.context_images.len(),
                tools_enabled = self.core.tools_enabled,
                "{}",
                miyu_base::i18n::text(
    ...
    pub(in crate::agent) async fn describe_inline_media(
        &self,
        items: Vec<miyu_core::state::TurnInlineMedia>,
    ) -> Result<Vec<miyu_core::state::TurnInlineMedia>> {
        let images = items
            .iter()
            .filter(|item| item.kind == miyu_core::state::INLINE_MEDIA_KIND_IMAGE)
            .filter_map(|item| {
                item.data
                    .clone()
        ...
        Ok(vec![miyu_core::state::TurnInlineMedia {
            call_id: String::new(),
            seq: 0,
            kind: miyu_core::state::INLINE_MEDIA_KIND_TEXT.to_string(),
    ...
    pub(in crate::agent) fn uploaded_attachment_image_parts(
        &self,
        attachments: &[miyu_core::state::UserAttachment],
    ) -> Vec<ChatContentPart> {
        attachments
            .iter()
