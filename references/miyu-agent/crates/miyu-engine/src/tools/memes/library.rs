...
pub fn current_persona_library(config: &AppConfig) -> String {
...
pub fn meme_ref_exists(paths: &MiyuPaths, meme: &MemeRef) -> Result<bool> {
    Ok(find_meme(paths, &meme.library, &meme.id)?.is_some())
}

pub async fn delete_meme_reference(
    meme: &MemeRef,
    config: &AppConfig,
    paths: &MiyuPaths,
) -> Result<()> {
...
pub fn sanitize_library(value: &str) -> String {
    let value = value
        .chars()
        .map(|ch| {
...
pub(crate) fn builtin_library_dir(library: &str) -> PathBuf {
    miyu_base::paths::resources::child_directory(
        miyu_base::paths::resources::ResourceKind::Memes,
        Path::new(library),
    )
}
