...
pub enum MemeCollectionOutcome {
...
pub(crate) struct MemeClassification {
    ...
    #[serde(default)]
    #[allow(dead_code)] // serde 字段,老索引里有它
...
pub(crate) async fn add_meme(args: Value, config: &AppConfig, paths: &MiyuPaths) -> Result<String> {
    ...
    miyu_base::sandbox::guard_read(&source)?;
...
pub(crate) fn parse_meme_classification(text: &str) -> Result<MemeClassification> {
    ...
    let Some(extracted) = miyu_base::json_extract::extract_json_object(trimmed) else {
        bail!(
...
pub async fn collect_meme_from_local_image(
    image: &Path,
    config: &AppConfig,
    paths: &MiyuPaths,
