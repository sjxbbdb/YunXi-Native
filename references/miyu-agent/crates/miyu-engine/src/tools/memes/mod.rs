...
pub use dashboard::*;
pub use library::*;
pub use semantic::reindex_library;
pub use validate::*;

use super::{vision, ToolRegistry, ToolSpec};
use anyhow::{anyhow, bail, Context, Result};
use image::AnimationDecoder;
use miyu_base::config::{AppConfig, MemesPluginConfig};
use miyu_base::paths::MiyuPaths;
use miyu_base::prompts::MEME_DESCRIPTION_PROMPT;
...
fn rank_memes(
    ...
    let ordered_ids: Vec<String> = match semantic_rank {
        Some(semantic_ids) if !semantic_ids.is_empty() => miyu_base::embedding::rrf_fuse(
            &[keyword_ids, semantic_ids],
            miyu_base::embedding::RRF_K,
        )
        .into_iter()
        .map(|(id, _)| id)
        .collect(),
