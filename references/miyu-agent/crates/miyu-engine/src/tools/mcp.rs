...
struct McpSession {
    ...
    next_id: u64,
    timeout: Duration,
    /// 宿主查询令牌的守卫:服务器进程活着令牌就有效,会话掉落即作废。
    _host_grant: Option<miyu_base::host_ports::HostGrantGuard>,
}

/// 服务器配置里声明的能力 ∩ 宿主词表;不认识的记 warn 丢掉(与脚本同一规则)。
fn host_capabilities_for_server(server: &McpServerConfig) -> Vec<String> {
    server
        .capabilities
        .iter()
        .filter(|id| {
...
"#;
        ...
        let tools = list_server_tools_inner(&server, None).unwrap();
...
"#,
    ...
    fn server_capabilities_are_filtered_by_the_host_vocabulary() {
        ...
        let mut server = mock_server("caps", &temp.path().join("marker"), 0.0, false);
        ...
        assert_eq!(
            host_capabilities_for_server(&server),
            vec!["providers.read".to_string(), "host.info".to_string()]
        );
        server.capabilities.clear();
        assert!(host_capabilities_for_server(&server).is_empty());
    }
