...
use miyu_base::clipboard::write_image_cache_file;
use miyu_base::config::{AppConfig, PrintImagePluginConfig};
use miyu_base::i18n::text as t;
pub use miyu_base::media_mime::{pdf_mime, video_mime};
...
use base64::Engine;
use miyu_base::platform_types::PlatformToolContext;
...
/// 免得寄存一个取走时才发现是坏的引用。
pub fn pdf_inline_media(path: &str) -> Result<Vec<miyu_core::state::TurnInlineMedia>> {
    let mime = pdf_mime(path).context("not a PDF path")?;
    let expanded = expand_path(path);
    miyu_base::sandbox::guard_read(&expanded)?;
    ...
    Ok(vec![miyu_core::state::TurnInlineMedia {
        call_id: String::new(),
        seq: 0,
        kind: miyu_core::state::INLINE_MEDIA_KIND_PDF.to_string(),
        mime: mime.to_string(),
...
async fn analyze_scoped_image_one(
    ...
    if state.context_images.contains_key(image) {
        ...
        if active_text_pool_for_vision(&config).is_some() {
            return Ok(inline::deposit(vec![miyu_core::state::TurnInlineMedia {
                call_id: String::new(),
                seq: 0,
    ...
    if image.starts_with("http://") || image.starts_with("https://") {
        ...
        if miyu_base::platform_types::is_trusted_avatar_url(image) {
            return analyze_image_one(args, config, paths).await;
        }
    ...
    miyu_base::sandbox::guard_read(&image)?;
...
pub(crate) fn vision_pool_timeout(
    vision: &miyu_base::config::VisionPluginConfig,
    endpoints: usize,
) -> u64 {
...
fn active_text_pool_for_vision(
    config: &AppConfig,
) -> Option<Vec<miyu_base::config::ProviderModelChoice>> {
...
fn try_inline_targets(config: &AppConfig, targets: &[String]) -> Result<Option<String>> {
    ...
    for target in targets {
        ...
        if let Some(mime) = video_mime(target) {
            ...
            items.push(miyu_core::state::TurnInlineMedia {
                call_id: String::new(),
                seq: 0,
                kind: miyu_core::state::INLINE_MEDIA_KIND_VIDEO.to_string(),
                mime: mime.to_string(),
                source: if remote {
                    target.to_string()
                } else {
