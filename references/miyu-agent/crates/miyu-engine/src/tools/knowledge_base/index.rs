...
impl KnowledgeBase {
    ...
    pub(in crate::tools::knowledge_base) async fn reindex_embeddings_inner(
        &self,
        embedder: &miyu_base::embedding::Embedder,
        quiet: bool,
        ...
        for record in files {
            ...
            for (chunk, vector) in chunks.iter().zip(vectors) {
                semantic.execute(
                    ...
                    params![embedder.describe(), model, record.name, record.content_sha256, chunk.index as i64, chunk.start as i64, chunk.end as i64, chunk.text, miyu_base::embedding::vector_to_blob(&vector), now_secs()],
                )?;
                indexed += 1;
            }
    ...
    pub(in crate::tools) fn spawn_embedding_reindex(&self) -> Result<()> {
        ...
        let exe = miyu_base::paths::miyu_executable()?;
    ...
    pub(in crate::tools::knowledge_base) fn embedder(
        &self,
    ) -> Option<miyu_base::embedding::Embedder> {
        if !self.config.plugins.knowledge_base.embedding_enabled {
            return None;
        }
        miyu_base::embedding::Embedder::from_config(&self.config)
    }
