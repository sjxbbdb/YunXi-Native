...
use miyu_base::i18n::{locale, Locale};
...
pub fn list_global_scripts(paths: &MiyuPaths) -> Vec<(String, String, String)> {
...
pub fn list_scripts_with_origin(
    dirs: &[&std::path::Path],
...
async fn run_script(
    ...
    // 宿主查询令牌(09-16):头部声明了 Capabilities 且在 daemon 里跑才有;守卫活到
    // 脚本退出,令牌随之作废。脚本用 `$MIYU_HOST_BIN host <method>` 查。
    let host_grant = miyu_base::host_ports::issue_host_grant(host_capabilities);
    if let Some(grant) = &host_grant {
        command.env("MIYU_HOST_TOKEN", grant.token());
        command.env("MIYU_HOST_CAPABILITIES", host_capabilities.join(","));
