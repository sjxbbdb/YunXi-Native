...
impl HomeLayout {
    ...
    pub fn plan(&self) -> Result<Vec<HomeMovePlan>> {
        self.entries()
            .into_iter()
            .map(|entry| {
                Ok(HomeMovePlan {
...
/// 数字),目录名直接用它。
pub fn is_valid_home_name(name: &str) -> bool {
...
pub fn admin_home_name_from_env() -> String {
...
/// 用户回滚过并要求别再自动搬。
pub fn home_layout_opted_out(root: &Path) -> Result<bool> {
    marker_exists_at(
...
pub fn read_home_layout_admin(root: &Path) -> Result<Option<String>> {
...
pub fn try_migrate_home_layout(
    layout: &HomeLayout,
...
pub fn rollback_home_layout(layout: &HomeLayout, current_process_is_daemon: bool) -> Result<bool> {
...
pub fn clear_home_layout_opt_out(layout: &HomeLayout) -> Result<()> {
