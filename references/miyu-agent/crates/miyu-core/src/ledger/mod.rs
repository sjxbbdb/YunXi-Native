...
impl LedgerDb {
    ...
    pub fn open_for(config: &miyu_base::config::AppConfig, paths: &MiyuPaths) -> Result<Self> {
    ...
    pub fn db_path_for(config: &miyu_base::config::AppConfig, paths: &MiyuPaths) -> PathBuf {
...
/// 把 RFC3339 时刻转成本地自然日。
pub fn local_day_of(rfc3339: &str) -> Result<String> {
