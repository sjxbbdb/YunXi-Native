...
#[derive(Clone)]
pub struct MemoryOrganizerHandle {
...
impl MemoryOrganizer {
        ...
        let join = std::thread::Builder::new()
            ...
            .spawn(move || {
                    ...
                    .build()
                {
                    Ok(runtime) => runtime,
                    Err(error) => {
                        tracing::error!(error = %error, "{}", miyu_base::i18n::text("building memory organizer runtime failed", "构建记忆整理器运行时失败"));
                        return;
                    }
                };
    ...
    pub fn handle(&self) -> MemoryOrganizerHandle {
        self.handle.clone()
    }
...
impl MemoryOrganizerHandle {
    pub fn wake(&self, config: AppConfig, paths: MiyuPaths, state_store: StateStore) {
    ...
    F: for<'a> FnMut(&'a OrganizerJob, bool) -> futures_util::future::BoxFuture<'a, Result<bool>>,
{
    ...
    loop {
        ...
        match process(&job, give_up).await {
            ...
            Ok(false) => {}
            Err(error) => {
                tracing::warn!(persona, error = %error, "{}", miyu_base::i18n::text("memory organization failed; batch remains pending", "记忆整理失败；批次仍待处理"));
...
async fn process_job(job: &OrganizerJob, give_up_on_failure: bool) -> Result<bool> {
    ...
    for _ in 0..MAX_BATCHES_PER_WAKE {
        ...
        if let Err(error) = applied {
            ...
            tracing::warn!(error = %error, diaries = batch.diaries.len(), "{}", miyu_base::i18n::text("giving up on a memory batch after repeated failures", "记忆批次连续失败,放弃这一批"));
...
async fn organize_batch(
    ...
    let client = OpenAiCompatibleClient::from_aux_role(
        &job.config,
        &job.paths,
        miyu_base::config::AuxRole::MemoryOrganizer,
    )
...
材料：\n{}",
    ...
    if let Some(usage) = result.usage.as_ref() {
        ...
        if let Err(error) = job.state_store.add_auxiliary_usage(usage, meta) {
            tracing::warn!(error = %error, "{}", miyu_base::i18n::text("recording memory organizer usage failed", "记录记忆整理器用量失败"));
        }
    }
