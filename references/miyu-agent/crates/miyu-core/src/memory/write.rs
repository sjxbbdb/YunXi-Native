    ...
    pub(crate) fn apply_organized_batch(
        ...
        output.knowledge.retain(|action| {
            ...
            if let Err(error) = &verdict {
                tracing::warn!(error = %error, content = %action.content, "{}", miyu_base::i18n::text("dropping one organized knowledge item", "丢弃一条不合规的整理知识点"));
            }
            verdict.is_ok()
        });
        output.long_diaries.retain(|diary| {
            ...
            if let Err(error) = &verdict {
                tracing::warn!(error = %error, content = %diary.content, "{}", miyu_base::i18n::text("dropping one organized long diary", "丢弃一条不合规的整理长期日记"));
            }
            verdict.is_ok()
        });
        ...
        if !forced_ids.is_subset(&promoted_ids) {
            ...
            // 待晋升标记,否则它会被无限次重新选中。
            tracing::warn!(missing = ?forced_ids.difference(&promoted_ids).collect::<Vec<_>>(), "{}", miyu_base::i18n::text("memory organizer did not promote every required diary", "记忆整理器没有晋升全部被点名的日记"));
        }
