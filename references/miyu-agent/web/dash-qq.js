...
(() => {
  ...
  const ui = {};
  const KIND_LABEL = { group: t("群"), private: t("私聊") };
  const MEDIA_LABEL = { image: t("图片"), sticker: t("表情"), file: t("文件"), audio: t("语音"), video: t("视频"), other: t("其他") };
  ...
  function mount(root) {
    ...
    ui.account = D.select([], state.account, (value) => { state.account = value; D.remember("qq.account", value); state.conversation = ""; loadConversations(); }, t("bot 账号"));
    ui.conversation = D.select([], state.conversation, (value) => { state.conversation = value; D.remember("qq.conversation", value); resetPaging(); loadTab(); }, t("会话"));
    const head = D.el("div.con-head", null,
      D.el("h2", { text: t("QQ 消息记录") }),
      D.iconButton("refresh-cw", t("刷新"), () => reloadAll()),
      ui.stamp,
      D.el("span.dash-scope", null, D.el("span.dash-scope-label", { text: t("账号") }), ui.account, D.el("span.dash-scope-label", { text: t("会话") }), ui.conversation));
    ui.cards = D.el("div");
    ui.tabs = D.segmented([{ value: "messages", label: t("消息") }, { value: "stats", label: t("统计") }, { value: "recalls", label: t("撤回") }], state.tab, (value) => { state.tab = value; resetPaging(); renderToolbar(); loadTab(); });
    ui.filters = D.el("div.dash-filters");
    ui.search = D.el("input.dash-search", { type: "search", placeholder: t("搜索消息(3 字以上走全文索引)…"), oninput: () => {
  ...
  function renderToolbar() {
    ...
    if (state.tab === "messages") {
      const sender = D.el("input.dash-select.dash-tag-input", { type: "text", placeholder: t("发送者 QQ"), value: f.sender, oninput: () => { clearTimeout(ui.senderTimer); ui.senderTimer = setTimeout(() => { f.sender = sender.value.trim(); resetPaging(); loadTab(); }, 300); } });
      ...
      const recalled = D.el(`button.dash-chip.is-clickable${f.recalled ? ".is-active" : ""}`, { type: "button", text: t("仅撤回"), onclick: () => { f.recalled = !f.recalled; resetPaging(); renderToolbar(); loadTab(); } });
      const media = D.el(`button.dash-chip.is-clickable${f.media ? ".is-active" : ""}`, { type: "button", text: t("含媒体"), onclick: () => { f.media = !f.media; resetPaging(); renderToolbar(); loadTab(); } });
      ui.filters.append(sender, start, D.el("span.dash-filter-sep", { text: "→" }), end, recalled, media);
    } else if (state.tab === "stats") {
      ui.filters.append(D.segmented([{ value: 7, label: t("7 天") }, { value: 30, label: t("30 天") }, { value: 90, label: t("90 天") }, { value: 0, label: t("全部") }].map((o) => ({ value: String(o.value), label: o.label })), String(state.days), (value) => { state.days = Number(value); loadTab(); }).el);
    }
  }
  ...
  async function loadAccounts() {
    try {
      ...
      for (const account of state.accounts) ui.account.append(D.el("option", { value: account, text: t("{account}{online}", { account, online: payload.connected?.includes(account) ? t("(在线)") : "" }) }));
      ui.account.value = state.account;
    } catch (error) {
      ui.stamp.textContent = t("账号列表加载失败:{error}", { error: error.message });
    }
  }

  async function loadConversations() {
    if (!state.account) { ui.cards.replaceChildren(D.el("p.dash-empty", { text: t("还没有任何 QQ 消息记录。") })); return; }
    try {
      ...
      ui.cards.replaceChildren(D.statCards([
        { label: t("会话"), value: state.conversations.length, hint: t("{groups} 群 · {privates} 私聊", { groups: state.conversations.filter((c) => c.kind === "group").length, privates: state.conversations.filter((c) => c.kind === "private").length }) },
        { label: t("本账号消息"), value: payload.total_messages, hint: t("库文件 {size} MB · 无自动清理", { size: mb }) },
        { label: t("当前会话"), value: current ? current.messages : 0, hint: current ? t("bot {bot} · 撤回 {recalled}", { bot: current.bot_messages, recalled: current.recalled }) : "—" },
        { label: t("时间跨度"), value: current ? t("{days} 天", { days: Math.max(1, Math.round((current.last_at - current.first_at) / 86400)) }) : "—", hint: current ? `${ts(current.first_at).slice(0, 10)} → ${ts(current.last_at).slice(0, 10)}` : "" }
      ]));
      resetPaging();
      await loadTab();
    } catch (error) {
      ui.stamp.textContent = t("会话列表加载失败:{error}", { error: error.message });
    }
  }

  async function loadTab() {
    if (!state.conversation) { ui.body.replaceChildren(D.el("p.dash-empty", { text: t("选择一个会话。") })); ui.pager.replaceChildren(); ui.danger.replaceChildren(); return; }
    renderDanger();
  ...
  async function loadMessages() {
    ...
    try {
      ...
      ui.stamp.textContent = t("{count} 条{mode}", { count: state.items.length, mode: state.q ? (payload.fts ? t(" · 全文索引") : t(" · 模糊匹配")) : "" });
    } catch (error) {
      if (seq !== state.loadSeq) return;
  ...
  function renderMessages(payload) {
    ...
    if (!state.items.length) {
      ui.body.append(D.el("p.dash-empty", { text: state.q ? t("没有匹配的消息。") : t("这个会话还没有消息。") }));
      ui.pager.replaceChildren();
      return;
    }
    const grid = D.table([{ label: t("时间"), width: "132px" }, { label: t("发送者"), width: "150px" }, { label: t("内容"), width: "minmax(300px, 4fr)" }, { label: t("媒体"), width: "120px" }, { label: "", width: "56px" }]);
    for (const m of state.items) {
      ...
      const flags = D.el("span", null, m.recalled_at ? D.el("span.dash-chip.is-danger", { text: t("撤回") }) : null);
      const row = D.el("div.dash-row", { role: "row", tabindex: "0", onclick: () => openMessage(m), onkeydown: (event) => { if (event.key === "Enter") openMessage(m); } },
        ...
        D.el("span.dash-cell-main", { text: m.text || (m.media?.length ? t("[非文本消息]") : "") }),
        media, flags);
    ...
    const newer = D.iconButton("chevron-left", t("较新"), () => { state.pages.pop(); state.cursor = state.pages[state.pages.length - 1] || null; loadMessages(); });
    const older = D.iconButton("chevron-right", t("较早"), () => { state.pages.push(payload.next_cursor); state.cursor = payload.next_cursor; loadMessages(); });
    newer.disabled = !state.pages.length;
    older.disabled = !payload.next_cursor;
    bar.append(D.el("span.dash-pager-text", { text: t("第 {page} 页 · 每页 50 · 新在前", { page: state.pages.length + 1 }) }), newer, older);
    ui.pager.replaceChildren(bar);
  }

  function openMessage(m) {
    const meta = [[t("消息 ID"), m.message_id], [t("发送者"), `${m.sender_name || "—"}(${m.sender_id})${m.is_bot ? " · bot" : ""}`], [t("时间"), ts(m.sent_at)]];
    if (m.reply_to) meta.push([t("回复"), m.reply_to]);
    if (m.recalled_at) meta.push([t("撤回于"), ts(m.recalled_at)]);
    ...
    const body = D.el("div", null,
      D.el("p.dash-detail-content", { text: m.text || t("[非文本消息]") }),
      ...
      D.el("p.dash-field-hint", { text: t("历史库只存媒体占位符,没有图片字节与路径。") }),
      D.el("dl.dash-meta", null, meta.flatMap(([key, value]) => [D.el("dt", { text: key }), D.el("dd", { text: String(value) })])));
    D.openDrawer(t("消息 {id}", { id: m.message_id }), body, []);
  }

  /* ── 统计 ─────────────────────────────────────────────── */
  async function loadStats() {
    const seq = ++state.loadSeq;
    ui.stamp.textContent = t("统计中…");
    ui.pager.replaceChildren();
    ...
    try {
      ...
      if (seq !== state.loadSeq) return;
      renderStats(s);
      ui.stamp.textContent = state.days ? t("近 {days} 天 {count} 条", { days: state.days, count: s.total }) : t("全部 {count} 条", { count: s.total });
    } catch (error) {
      if (seq !== state.loadSeq) return;
      ui.body.replaceChildren(D.el("p.dash-empty", { text: t("统计失败:{error}", { error: error.message }) }));
    }
  }
  ...
  function renderStats(s) {
    ...
    const podium = D.el("div.dash-podium", null, r.items.slice(0, 3).map((i, index) => D.el("div.dash-podium-item", null,
      ...
      D.el("span.dash-cell-muted", { text: t("{messages} 条 · {days} 天活跃", { messages: i.message_count, days: i.active_days }) }))));
    ...
    const rankCard = D.el("div.u-card", null, D.el("div.u-card-head", null, D.el("h3", { text: t("发言榜") }), D.el("span.u-hint", { text: t("{count} 人参与 · bot 计入", { count: r.participant_count }) })), podium, bars.childElementCount ? bars : D.el("p.dash-empty", { text: t("这段时间没有消息。") }));
    ...
    s.heat.forEach((row, d) => {
      ...
      row.forEach((count, h) => {
        ...
        heat.append(D.el("i", { dataset: { l: String(level) }, title: t("周{day} {hour}:00 · {count} 条", { day: days[d], hour: h, count }) }));
      });
    });
    const heatCard = D.el("div.u-card", null, D.el("div.u-card-head", null, D.el("h3", { text: t("活跃时段") }), D.el("span.u-hint", { text: t("星期 × 小时,本地时间") })), heat);

    const media = Object.entries(s.media || {}).sort((a, b) => b[1] - a[1]);
    const mixCard = D.el("div.u-card", null, D.el("div.u-card-head", null, D.el("h3", { text: t("构成") })),
      D.el("div.dash-kv", null,
        D.el("span", { text: t("人类") }), D.el("strong", { text: String(s.human) }),
  ...
  async function loadRecalls() {
    ...
    try {
      ...
      if (seq !== state.loadSeq) return;
      ui.body.textContent = "";
      if (!payload.items.length) { ui.body.append(D.el("p.dash-empty", { text: t("没有撤回记录。") })); ui.pager.replaceChildren(); ui.stamp.textContent = t("0 条"); return; }
      const grid = D.table([{ label: t("撤回时间"), width: "132px" }, { label: t("操作者"), width: "120px" }, { label: t("发送者"), width: "150px" }, { label: t("原文"), width: "minmax(300px, 4fr)" }]);
      for (const r of payload.items) {
        grid.append(D.el("div.dash-row", { role: "row" },
          ...
          D.el("span", { text: r.sender_name ? `${r.sender_name}(${r.sender_id})` : (r.sender_id || t("未知")) }),
          D.el("span.dash-cell-main", { text: r.text ?? t("(原消息未记录)") })));
      }
      ui.body.append(grid);
      ...
      ui.stamp.textContent = t("{count} 条撤回", { count: payload.total });
    } catch (error) {
      if (seq !== state.loadSeq) return;
      ui.body.replaceChildren(D.el("p.dash-empty", { text: t("加载失败:{error}", { error: error.message }) }));
    }
  ...
  function renderDanger() {
    ...
    ui.danger.replaceChildren(D.el("div.dash-danger", null,
      D.el("h4.dash-section", { text: t("维护") }),
      D.el("div.dash-actions", null,
        D.el("button.dash-button", { type: "button", text: t("重置上下文边界"), title: t("让当前人格从现在起不再读取这个会话更早的历史"), onclick: resetBoundary }),
        D.el("button.dash-button.is-danger", { type: "button", text: t("删除本会话历史…"), onclick: () => openDelete(false) }),
        D.el("button.dash-button.is-danger", { type: "button", text: t("删除账号 {account} 全部群聊历史…", { account: state.account }), onclick: () => openDelete(true) })),
      D.el("p.dash-field-hint", { text: t("当前会话:{kind} {id}。历史库没有自动保留策略,这里是唯一的清理入口。", { kind: KIND_LABEL[c.kind] || c.kind, id: c.id }) })));
  }

  async function resetBoundary() {
    const c = conv();
    const ok = await D.confirmAction(t("重置 {kind} {id} 的上下文边界?当前人格之后只会读到这一刻之后的消息,历史本身不删。", { kind: KIND_LABEL[c.kind], id: c.id }), t("重置"));
    if (!ok) return;
    try {
  ...
  function openDelete(wholeAccount) {
    const c = conv();
    const target = wholeAccount ? t("账号 {account} 下全部群聊", { account: state.account }) : `${KIND_LABEL[c.kind]} ${c.id}`;
    const expected = wholeAccount ? state.account : c.id;
    let mode = "all";
    const modeSeg = D.segmented([{ value: "all", label: t("全部") }, { value: "keep", label: t("只保留最近 N 天") }, { value: "range", label: t("按条件") }], mode, (value) => { mode = value; keepBox.hidden = value !== "keep"; rangeBox.hidden = value !== "range"; });
    const keepDays = D.el("input.dash-select.dash-wide", { type: "number", min: "1", value: "30" });
    const keepBox = D.el("div", { hidden: true }, D.field(t("保留天数"), keepDays));
    const sender = D.el("input.dash-select.dash-wide", { type: "text", placeholder: t("发送者 QQ(可选)") });
    const start = D.el("input.dash-select.dash-wide", { type: "date" });
    const end = D.el("input.dash-select.dash-wide", { type: "date" });
    const rangeBox = D.el("div", { hidden: true }, D.field(t("发送者"), sender), D.el("div.dash-field-row", null, D.field(t("起"), start), D.field(t("止"), end)));
    const confirm = D.el("input.dash-select.dash-wide", { type: "text", placeholder: expected });
    const body = D.el("div", null,
      D.el("p.dash-banner", { text: t("即将删除 {target} 的消息记录。这不可恢复,也不会通知模型。", { target }) }),
      D.field(t("范围"), modeSeg.el), keepBox, rangeBox,
      D.field(t("确认"), confirm, t("输入 {text} 以确认", { text: expected })));
    const submit = D.el("button.dash-button.is-danger", { type: "button", text: t("删除"), onclick: async () => {
      if (confirm.value.trim() !== expected) { D.toast(t("确认文字不匹配"), "error"); return; }
      ...
      try {
        ...
        D.toast(t("已删除 {messages} 条消息 · {recalls} 条撤回", { messages: result.messages_deleted, recalls: result.recalls_deleted }));
        D.closeDrawer();
        await loadConversations();
      } catch (error) {
    ...
    D.openDrawer(t("删除历史"), body, [submit]);
  }
