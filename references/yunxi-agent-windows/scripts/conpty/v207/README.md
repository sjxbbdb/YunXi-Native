# v2.0.7 Windows ConPTY Evidence Collector
...
archive hash. Installation remains below `scripts/conpty/v207/node_modules`;
...
node -e "require('./scripts/conpty/v207/node_modules/node-pty'); console.log('node-pty binding ready')"
...
```powershell
npm run capture:scenario --prefix scripts\conpty\v207 -- stream-cancel
```
...
`npm run manifest --prefix scripts\conpty\v207` rebuilds only the SHA-256
...
The ignored `scripts/conpty/v207/.work` directory contains only temporary
